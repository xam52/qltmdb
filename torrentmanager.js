(function () {
    ...
    function getPosterFromLabels(labels) {
      return new Promise(function (resolve, reject) {
        ...
        // Используем русский язык для получения постеров
        var language = 'ru-RU';
        var directTMDBApi = `https://api.themoviedb.org/3/${type}/${id}/images?api_key=${Lampa.TMDB.key()}&language=${language}`;
        ...
        if (Lampa.Storage.field('lmetorrentproxyTMDB') == true) {
          proxyTMDBApi = `https://lampame.v6.rocks?destination=https://api.themoviedb.org/3/${type}/${id}/images?api_key=${Lampa.TMDB.key()}&language=${language}`;
        ...
        $.ajax({
          ...
          success: function success(response) {
            try {
              ...
              // Если нет постера на русском, попробуем английский
              if (!poster || !poster.file_path) {
                // Повторяем запрос на английском
                var englishApi = Lampa.Storage.field('lmetorrentproxyTMDB') == true 
                  ? `https://lampame.v6.rocks?destination=https://api.themoviedb.org/3/${type}/${id}/images?api_key=${Lampa.TMDB.key()}&language=en-US`
                  : `https://api.themoviedb.org/3/${type}/${id}/images?api_key=${Lampa.TMDB.key()}&language=en-US`;
                ...
                $.ajax({
                  ...
                  success: function(englishResponse) {
                    processPosterResponse(englishResponse, label, resolve);
                  },
                  error: function() {
                    // Если и английский не сработал, используем дефолтное изображение
                    cacheDefaultPoster(label, resolve);
                  }
                });
              } else {
                processPosterResponse(response, label, resolve);
              }
            } catch (error) {
              cacheDefaultPoster(label, resolve);
            }
    ...
    // Вспомогательная функция для обработки ответа с постером
    function processPosterResponse(response, label, resolve) {
      try {
        var poster = response.posters && response.posters[0];
        if (poster && poster.file_path) {
          ...
          if (Lampa.Storage.field('lmetorrentproxyTMDB') == true) {
            imageUrl = 'https://lampame.v6.rocks?destination=https://image.tmdb.org/t/p/' + posterSize + poster.file_path;
          } else {
            imageUrl = 'https://image.tmdb.org/t/p/' + posterSize + poster.file_path;
          }
    ...
    // Вспомогательная функция для кэширования дефолтного изображения
    function cacheDefaultPoster(label, resolve) {
    ...
    // Функция поиска нескольких фильмов с русским языком
    function searchMultipleMovies(query) {
      return new Promise(function(resolve, reject) {
        var searchUrl = proxy$1 + "https://api.themoviedb.org/3/search/movie?query=" + 
                        ...
                        "&page=1&language=ru-RU&include_adult=false";
    ...
    // Функция поиска нескольких сериалов с русским языком
    ...
    // Функция поиска фильма с русским языком (для упрощенной версии)
    function searchMovie(query) {
      return new Promise(function(resolve, reject) {
        var searchUrl = proxy$1 + "https://api.themoviedb.org/3/search/movie?query=" + 
                        encodeURIComponent(query) + "&api_key=" + Lampa.TMDB.key() + 
                        "&language=ru-RU&include_adult=false";
    ...
    // Функция поиска сериала с русским языком (для упрощенной версии)
    ...
    //функцию поиска с альтернативным поиском на английском
      ...
      return new Promise(function(resolve, reject) {
        ...
        Promise.all([
          ...
          if (allResults.length === 0) {
            // Если не нашли на русском, пробуем на английском
            console.log('No results in Russian, trying English...');
            return searchInEnglish(cleanName);
          }
          return allResults;
        }).then(function(allResults) {
          if (allResults.length === 0) {
            // Если не нашли и на английском, пробуем альтернативный поиск без года
            ...
            if (nameWithoutYear !== cleanName) {
              return Promise.all([
                ...
                if (altAllResults.length === 0) {
                  Lampa.Bell.push({
                    text: 'Не найдено результатов для: ' + cleanName
                  });
          ...
          if (media) {
            ...
            Lampa.Bell.push({
              text: 'Добавлено: ' + (media.title || media.name)
            });
          }
          resolve(media);
          ...
          Lampa.Bell.push({
            text: 'Не удалось найти метаданные для: ' + cleanName
          });
          reject(error);
        });
      });
    ...
    // Функция поиска на английском языке
    function searchInEnglish(query) {
      return new Promise(function(resolve, reject) {
        var searchUrlMovies = proxy$1 + "https://api.themoviedb.org/3/search/movie?query=" + 
                             encodeURIComponent(query) + "&api_key=" + Lampa.TMDB.key() + 
                             "&page=1&language=en-US&include_adult=false";
        
        var searchUrlTV = proxy$1 + "https://api.themoviedb.org/3/search/tv?query=" + 
                         encodeURIComponent(query) + "&api_key=" + Lampa.TMDB.key() + 
                         "&page=1&language=en-US&include_adult=false";
        
        Promise.all([
          $.ajax({url: searchUrlMovies, method: "GET", headers: getHeaders$3()}),
          $.ajax({url: searchUrlTV, method: "GET", headers: getHeaders$3()})
        ]).then(function(results) {
          var movies = results[0].results || [];
          var tvShows = results[1].results || [];
          
          // Добавляем тип медиа
          movies = movies.slice(0, 3).map(function(movie) {
            movie.media_type = 'movie';
            return movie;
          });
          
          tvShows = tvShows.slice(0, 3).map(function(tv) {
            tv.media_type = 'tv';
            return tv;
          });
  	...
  	function showMediaSelectionDialog(results, torrentData) {
  	  return new Promise(function(resolve, reject) {
  	    ...
  	    items.push({
  	      title: 'Пропустить - не добавлять метаданные',
  	      description: 'Отменить добавление метаданных для этого торрента',
  	    ...
  	    Lampa.Select.show({
  	      title: 'Выберите правильное название для: ' + torrentData.name,
  	      items: items,
  	...
  	// Улучшенная функция очистки названия торрента с поддержкой русского
    function cleanTorrentName(name) {
      ...
      cleaned = cleaned
        .replace(/([Сс]езон\s*\d+|[Сс]ерии|[Ээ]пизод|[Сс]ез\.?\s*\d*|[Сс]е?\.?\s*\d+)/g, ' ') // Русские обозначения сезонов
        .replace(/([Ss](\d+)[Ee](\d+))/g, ' ') // Английские обозначения сезонов
      ...
      // Удаляем лишние символы, но сохраняем русские буквы
      cleaned = cleaned
        .replace(/[\[\]\(\)]/g, ' ')
    ...
    function SendCommand$3(btn, torrent_data) {
	  return new Promise(function (resolve, reject) {
	    // ОБРАБОТКА ДЕЙСТВИЙ PARSE И PARSE-ALL
	    if (btn.action === 'parse' || btn.action === 'parse-all') {
	      ...
	      if (btn.action === 'parse-all') {
	        // Для всех торрентов
	        Lampa.Bell.push({
	          text: 'Начинаю поиск метаданных для всех торрентов...'
	        });
	        ...
	        GetData$3().then(function(allTorrents) {
	          ...
	          var processNext = function(index) {
	            if (index >= total) {
	              ...
	              Lampa.Bell.push({
	                text: 'Поиск метаданных завершен: ' + successful + '/' + total + ' successful'
	              });
	              resolve();
	              return;
	            }
	          ...
	          Lampa.Bell.push({
	            text: 'Ошибка получения списка торрентов'
	          });
	          resolve();
	        });
	        ...
	        // Для одного торрента
	        Lampa.Bell.push({
	          text: 'Поиск метаданных для: ' + torrent_data.name
	        });
	        
	        searchAndAddMetadata(torrent_data).then(function(media) {
	          if (media) {
	            Lampa.Bell.push({
	              text: 'Добавлены метаданные: ' + (media.title || media.name)
	            });
	          }
	          resolve();
	        }).catch(function(error) {
	          ...
	          Lampa.Bell.push({
	            text: 'Не удалось найти метаданные для: ' + torrent_data.name
	          });
